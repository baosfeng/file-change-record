...
- **资源监控 + 自动降级（资源看门狗）**：面板「资源」区块展示本进程 CPU/内存 + 审计文件大小与写入速率（15s 采样），写放大/超限自动告警（阈值见 `resource-budget-review`）；写入速率/文件大小连续超限自动**暂停落盘**（事件仍入内存，有界不丢），回落自动恢复（全量快照补齐降级窗口），全程日志 + 「资源」区块降级标记（issue #127）。
