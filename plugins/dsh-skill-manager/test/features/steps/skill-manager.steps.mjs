/**
 * Step definitions for dsh-skill-manager Gherkin acceptance tests.
 * Boots the plugin against a mocked ctx per scenario (fresh world), drives
 * the /skill-manager/api routes through it, mirroring test/host-api.mjs.
 */
...
class World {
  constructor() {
    ...
    this.apiHolder = captureRoute('/skill-manager/api')
    this.invalidated = 0
...
When('以项目路径 {string} 查询 skill 列表', async function (cwd) {
  await this.callRoute('GET', `/skill-manager/api/list?cwd=${encodeURIComponent(cwd)}`)
})
...
When('通过刷新接口重新扫描', async function () {
  await this.callRoute('GET', '/skill-manager/api/rescan?cwd=')
})
...
When('查询全局 skill 列表', async function () {
  await this.callRoute('GET', '/skill-manager/api/list?cwd=')
})
...
Given('已保存全局禁用名单 {string}', async function (jsonList) {
  ...
  await this.callRoute('PUT', '/skill-manager/api/config', { scope: 'global', disabled })
})
