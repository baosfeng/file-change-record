/**
 * dsh-plugin-manager — client half (browser). SOURCE TEMPLATE.
 *
 ...
 *
 * Data source: GET/POST /plugin-manager/api/* (server half). Styling follows
...
`.trim()
    ...
    const API_BASE = '/plugin-manager/api'
    ...
    exports.apply = function apply(ctx) {
      ...
      ctx.effect(() => slots.inject('settings.plugins.tab', () => slots.register({
        ...
        order: 100,
        label: () => strings.title(),
      }, PluginManagerView)), 'dsh-plugin-manager: settings tab registration')
    }
