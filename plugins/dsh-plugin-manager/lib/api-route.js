/**
 * dsh-plugin-manager — /plugin-manager/api route handler.
 *
 *  - GET  /plugin-manager/api/installed  → loader 已安装清单 + 版本；
 *  - GET  /plugin-manager/api/search?q=… → npm registry 市场搜索；
 *  - POST /plugin-manager/api/install    → `dsh plugin --profile <p> add`;
 *  - POST /plugin-manager/api/uninstall  → `dsh plugin --profile <p> remove`;
 *  - GET  /plugin-manager/api/updates    → `pnpm outdated --json`（更新检查）。
  ...
  return async (request, response) => {
    ...
    try {
      ...
      if (spec === undefined || spec.method !== request.method) {
        writeJson(response, 404, { ok: false, error: { message: 'unknown plugin-manager API method' } })
        return
      }
...
/** Strip the /plugin-manager/api/ prefix; undefined for anything else. */
