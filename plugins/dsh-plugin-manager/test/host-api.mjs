/**
 * dsh-plugin-manager — API route + apply() integration tests.
 *
...
async function boot(overrides) {
  const apiHolder = captureRoute('/plugin-manager/api')
  const ctx = {
    logger: { warn: () => {} },
...
test('apply registers the API route', async () => {
  ...
  assert.ok(getRoute(), '/plugin-manager/api route registered')
})

test('API refuses requests outside the fence (403)', async () => {
  const { getRoute } = await boot()
  const res = makeResponse()
  await getRoute().handler(makeRequest('GET', '/plugin-manager/api/installed', undefined, {
...
test('GET /search calls the npm registry and clamps size', async () => {
  const { getRoute } = await boot()
  const r = await callRoute(getRoute, 'GET', '/plugin-manager/api/search?q=dsh-file&size=999')
  assert.equal(r.status, 200)
  assert.equal(r.json.value.results[0].name, 'dsh-x')
  const empty = await callRoute(getRoute, 'GET', '/plugin-manager/api/search?q=')
...
test('GET /updates surfaces outdated entries', async () => {
  ...
  const { getRoute } = await boot()
  const r = await callRoute(getRoute, 'GET', '/plugin-manager/api/updates')
  assert.equal(r.status, 200)
...
test('POST /install and /uninstall route through the CLI wrapper', async () => {
  const { getRoute } = await boot()
  const bad = await callRoute(getRoute, 'POST', '/plugin-manager/api/install', { source: '  ' })
  assert.equal(bad.status, 400, 'blank source rejected')

  const ok = await callRoute(getRoute, 'POST', '/plugin-manager/api/install', { source: 'dsh-x' })
  assert.equal(ok.status, 200)
  assert.equal(ok.json.ok, true)
  assert.ok(manageMock.installPlugin.mock.calls.some((call) => call[0] === 'web' && call[1] === 'dsh-x'))

  const un = await callRoute(getRoute, 'POST', '/plugin-manager/api/uninstall', { name: 'dsh-x' })
...
test('install/uninstall failures carry an error message', async () => {
  ...
  const { getRoute } = await boot()
  const r = await callRoute(getRoute, 'POST', '/plugin-manager/api/install', { source: 'dsh-bad' })
  assert.equal(r.status, 200)
...
test('unknown API methods return 404', async () => {
  const { getRoute } = await boot()
  const r = await callRoute(getRoute, 'GET', '/plugin-manager/api/nope')
...
test('fence: non-loopback hosts, origin mismatch and trusted hosts', async () => {
  ...
  const holder = captureRoute('/plugin-manager/api')
  const ctx = {
    logger: { warn: () => {} },
  ...
  await holder.get().handler(makeRequest('GET', '/plugin-manager/api/installed', undefined, {
...
test('handler errors are answered with a 400 JSON body', async () => {
  ...
  const res = makeResponse()
  await getRoute().handler(makeRequest('POST', '/plugin-manager/api/install', { source: huge }), res)
