...
async function shutdown(ctx) {
  const teardown = (ctx.fakeEffects ?? []).find((e) => e.label === 'dsh-guardian: teardown')
  teardown?.disposer()
...
try {
  // ── 1. staged entry mounts and gets PROMOTED (removed from the file) ────
  {
    ...
    assert.ok(state.events.some((e) => e.type === 'promote'), 'promote event logged')
    await shutdown(ctx1)
  }
  ...
  // ── 2. failing entry is quarantined (recorded, not promoted) ────────────
  {
    ...
    assert.deepEqual(JSON.parse(readFileSync(stagedFile(), 'utf8')), [{ id: 'bad-plugin', name: 'dsh-bad' }], 'candidate file untouched on failure')
    await shutdown(ctx2)
  }
  ...
  // ── 4. safe mode skips everything ───────────────────────────────────────
  {
    ...
    writeFileSync(stateFile(), JSON.stringify({ version: 1, safeMode: true, staged: {}, promoted: {}, events: [] }), 'utf8')
    ...
    assert.ok(state.events.some((e) => e.type === 'safe'), 'safe-mode skip event logged')
    await shutdown(ctx4)
  }
  ...
  // ── 7. restart recovery: promoted entries remount ───────────────────────
  {
    ...
    writeFileSync(stateFile(), JSON.stringify({
      ...
      safeMode: false,
      staged: {},
      promoted: { 'old-1': { name: 'dsh-old', config: undefined, attempts: 0, lastError: null, lastFailedAt: null, frozen: false, promotedAt: 1 } },
      events: [],
    }), 'utf8')
  ...
  // ── 8. API surface: state / staged / remove / safemode / fence / 404 ────
  {
    ...
    const add = await callApi(fake, 'POST', 'staged', { id: 'via-api', name: 'dsh-via', config: { k: 'v' } })
    assert.equal(add.status, 200)
    assert.equal(add.json.value.promoted.some((e) => e.id === 'via-api'), true, 'api-added entry promoted')
  ...
  // ── 9. teardown unmounts everything the guardian mounted ────────────────
  {
    ...
    await sleep(100)
    const teardown = ctx9.fakeEffects.find((e) => e.label === 'dsh-guardian: teardown')
