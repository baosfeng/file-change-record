...
- **最近访问**：按时间倒序记录文件读取 / 新增 / 修改事件（agent 工具读写 + 侧边栏打开/编辑保存都会记录），**最多保留最近 10 条**；点击任意文件即弹出**浮窗预览**（图片 / PDF / 文本 / 代码等，见下方「浮窗预览」）。
- **文件统计**：每个文件的「读取 / 新增 / 修改」次数，按文件**绝对路径**组织成**树形目录**；连续单子目录的路径链自动**压缩为点号标签**（如 `a/b/c` → `a.b.c`），文件直接列在下方；每个文件行内显示**最近访问时间**（相对时间），悬停可查看**创建时间**：
