...
/* ── sidebar tab selected-state: keep the host's original styling ─────────
   The host (dsh-better-sidebar) renders tabs with CSS-modules hashed class
   names (e.g. tabActive_xxxxx) and no data attributes. We previously injected
   a [class*=tab][class*=tabActive] override (issue #25) that filled the
   selected tab with the brand color (light #4176e6, dark #679efe). That
   colored the active sidebar tab blue, which issue #60 reports as unwanted:
   the user wants the original host state. So these overrides are removed, and
   the host's own .tabActive (grey interactive-bg-active background + primary
   label ink) is used for the selected tab — remaining distinguishable from the
   unselected tabs while no longer forcing a blue highlight. */
`
