...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const strings = {
      ...
      noSessions: () => (isZh() ? '暂无会话统计——开始一段对话后，上下文占用会出现在这里' : 'No session stats yet — context usage will appear here after a conversation'),
    ...
    function Stat({ value, label }) {
      return createElement('div', { className: 'dso-stat' },
    ...
    function windowNote(session) {
      ...
      return createElement('div', { className: 'dso-feedback' }, `${strings.contextWindow()} ${session.contextWindow.toLocaleString()}`)
    }
    ...
    function OverviewCard({ session }) {
      ...
      return createElement('div', { className: 'dso-card' },
        createElement('div', { className: 'dso-card-head' },
          createElement('span', { className: 'dso-card-title' }, strings.overview()),
          modelBadge(session),
        ),
        createElement('div', { className: 'dso-stat-row' },
          createElement(Stat, { value: strings.tokens(total), label: strings.totalTokens() }),
          createElement(Stat, { value: strings.percent(cacheHitRate(usage)), label: strings.cacheHitRate() }),
        ),
        createElement('div', { className: 'dso-stat-row' },
          createElement(Stat, { value: strings.tokens(usage.inputTokens || 0), label: strings.inputTokens() }),
          createElement(Stat, { value: strings.tokens(usage.outputTokens || 0), label: strings.outputTokens() }),
          createElement(Stat, { value: strings.tokens(usage.cacheReadTokens || 0), label: strings.cacheRead() }),
        ),
        windowNote(session),
      )
    }
    ...
    function CompositionBar({ composition }) {
      ...
      const rows = keys
        ...
        .map((key) => {
          ...
          return createElement('div', { key, className: 'dso-comp-row' },
            createElement('span', { className: 'dso-comp-label' }, compositionLabel(key)),
            createElement('div', { className: 'dso-comp-track' },
    ...
    function RequestRow({ request }) {
      ...
      return createElement('div', { className: 'dso-request' },
        createElement('div', { className: 'dso-request-head' },
        ...
        createElement('div', { className: 'dso-request-meta' },
    ...
    function BudgetSettings({ budget, onSaved }) {
      ...
      const save = () => void saveBudget({
        perTurn: Number(perTurn) || 0,
        perSession: Number(perSession) || 0,
        mode,
      }, setBusy, setFeedback, onSaved)
      return createElement('div', { className: 'dso-section' },
        createElement('div', { className: 'dso-section-title' }, strings.budget()),
        createElement(BudgetField, { label: strings.budgetPerTurn(), value: perTurn, onChange: setPerTurn }),
        createElement(BudgetField, { label: strings.budgetPerSession(), value: perSession, onChange: setPerSession }),
        createElement('div', { className: 'dso-repo-row' },
          createElement('select', { className: 'dso-select dso-budget-mode', value: mode, onChange: (e) => setMode(e.target.value) },
    ...
    function AlertList({ alerts }) {
      ...
      const rows = alerts.map((alert) => {
        ...
        return createElement('div', { key: alert.id, className: `dso-alert dso-alert-${alert.blocked ? 'danger' : 'warn'}` },
          ...
          createElement('div', { className: 'dso-alert-msg' },
    ...
    function sessionSections(session) {
      ...
      return [
        createElement('div', { key: 'comp', className: 'dso-section' },
    ...
    function ContextPanel(props) {
      ...
      return createElement('div', { className: 'dso-panel' },
        createElement('div', { className: 'dso-toolbar' },
          createElement('select', { className: 'dso-select', value: sessionId, onChange: (e) => setSessionId(e.target.value) },
