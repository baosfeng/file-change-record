...
验证时间：2026-09-04T13:25:36.691Z
验证环境：隔离实例（端口 3089，复用生产 profile 配置组合，独立 DSH_HOME）
...
### 验证记录（2026-09-04，隔离实例 3089 + 真实浏览器）
...
- **client UI（#90 详情页）**：设置 → 插件 → 插件管理 tab 正常渲染；点击「详情」打开 dsh-md-render 详情面板——README 预览（MarkdownView 渲染全文）、版本历史时间线（0.1.1/0.1.2 带 npm time 日期）、依赖区块（peer：cordis ^4.0.0-rc.8、react ^18.2.0 || ^19.2.0）、元数据（许可证/月下载量 tag）、版本选择器 + 详情内一键安装按钮都是真实验证到的。
