...
| [dsh-my-guard](plugins/dsh-my-guard/README.md)                   | 0.1.3 | 安全护栏：执行前护栏（破坏性命令 rm -rf / 等执行前拦截/确认，observe/ask/deny 三模式）、安装前投毒扫描（`dsh plugin add` 自动扫描包内容：可疑脚本/密钥/恶意依赖告警）、提示注入检测（规则 + 启发式检测 prompt injection / jailbreak，命中告警）；告警持久化 + 侧边栏安全护栏面板（确认机制）                                                                                                                  |
| [dsh-my-context](plugins/dsh-my-context/README.md)               | 0.1.2 | 上下文透镜 + 成本治理：token 用量可视化（每次请求的上下文构成、KV 缓存命中率，按会话隔离、重启后恢复）、每轮/每会话预算控制（token 上限，超限提醒/拦截，可配置）                                                                                                                                                                                                                                              |
