...
| [dsh-my-observability](plugins/dsh-my-observability/README.md)   | 0.1.3 | 可观测性 + Git 工程工具：事件审计（监听 agent/status、llm/stream、tools/*，agent 行为可追溯，按会话隔离、重启后恢复）、侧边栏轨迹回放时间轴面板（关键词搜索 + 组合过滤 + 命中高亮 + JSON/CSV 导出 + 工具统计）、结构化 Git 类型化提交（Conventional Commits）、提交前增量 diff 审查（规则引擎 + 可选 AI 审查）   |
