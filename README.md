...
| [dsh-md-render](plugins/dsh-md-render/README.md)                 | 0.1.4 | 非思考模式 markdown 表格渲染增强：模型输出的表格（含无首尾管道符、分隔行变体等不标准格式）自动识别并渲染为表格（表头/边框/对齐），宽表格横向滚动；兼容 dsh-think-zh-expand（tzx-md 容器）与内置 MarkdownText（md-table-wide 容器），思考模式表格渲染不受影响                                                                                                                                                  |
