...
| [dsh-my-memory](plugins/dsh-my-memory/README.md)                 | 0.1.4 | 记忆：全局/项目两级记忆持久化（全局 `$DSH_HOME/memory.json` + 项目 `$DSH_HOME/memory/projects/<项目 id>.json`（issue #108 集中存储 + 旧数据自动迁移）），会话开始时全局记忆注入系统提示词（agent 始终携带关键偏好，条数/长度上限防膨胀）；设置页面板（全局/项目分区 + 自定义确认 UI：删除红色、保存绿色，写操作必须用户确认）；`memory_query` 只读查询工具 + `memory_save` 保存工具（经用户确认，issue #107） |
