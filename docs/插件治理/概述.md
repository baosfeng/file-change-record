---
title: 概述
description: dsh-guardian 插件 — DSH 插件隔离与失败兜底机制（两段式加载/失败自动禁用/安全模式/诊断面板）
created: 2026-08-23
updated: 2026-08-23
---
...
| 业务关键词 | 入口与方法（源码路径） |
|-----------|----------------------|
| 候选插件加载 / 失败自动禁用 / 转正 | `plugins/dsh-guardian/lib/index.js`（server）— `apply()` 内 `scanStaged()` / `mountWithState()` / `processStagedEntry()` |
| 安全模式 | `plugins/dsh-guardian/lib/index.js` — `state.safeMode` 读取与生效（挂载跳过 + 卸载已挂载） |
| 诊断面板 / 状态列表 / 重试 | `plugins/dsh-guardian/lib/client.js`（client）— 侧边栏页签 + `/guardian/api/*` |
...
![插件守护面板：候选失败隔离 + 转正运行中 + 安全模式开关](../../plugins/dsh-guardian/assets/panel-main.png)

![失败自动隔离：错误详情可查](../../plugins/dsh-guardian/assets/panel-error-detail.png)

> 截图来自隔离 DSH 验证实例（3081 端口），完整说明见 [dsh-guardian README](../../plugins/dsh-guardian/README.md)。

## 依赖

- DSH web profile；可选 `dsh-better-sidebar`（诊断面板）、`dsh-notify`（失败通知）
