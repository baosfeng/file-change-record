...
| 业务关键词 | 入口与方法（源码路径） |
|-----------|----------------------|
| 候选插件加载 / 失败自动禁用 / 转正 | `plugins/dsh-guardian/lib/index.js`（server）— `loadStaged()` / `mountEntry()` / `quarantineEntry()` |
| 安全模式 | `plugins/dsh-guardian/lib/index.js` — `safeMode` 读取与生效 |
| 诊断面板 / 状态列表 / 重试 | `plugins/dsh-guardian/lib/client.js`（client）— 侧边栏页签 + RPC |
